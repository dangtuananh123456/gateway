...
func TestHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantStrings: []string{`"service":"mock-users"`, `"status":"ok"`},
		},
		{
			name:        "list users",
			method:      http.MethodGet,
			target:      "/",
			wantStatus:  http.StatusOK,
			wantStrings: []string{`"name":"Ada"`, `"name":"Linus"`},
		},
		{
			name:        "get user",
			method:      http.MethodGet,
			target:      "/42",
			wantStatus:  http.StatusOK,
			wantStrings: []string{`"id":"42"`, `"name":"User 42"`},
		},
		{
			name:        "mock error",
			method:      http.MethodGet,
			target:      "/_mock/error?status=503",
			wantStatus:  http.StatusServiceUnavailable,
			wantStrings: []string{`"code":"mock_error"`, `"message":"Service Unavailable"`},
		},
		{
			name:        "invalid mock status",
			method:      http.MethodGet,
			target:      "/_mock/error?status=200",
		...
		},
		{
			name:        "invalid delay",
			method:      http.MethodGet,
			target:      "/_mock/delay?duration=invalid",
			wantStatus:  http.StatusBadRequest,
